...
class Api::V1::TransactionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index action does not cause N+1 queries when rendering transfer transactions" do
    ...
    # With full eager loading, queries should be a small, bounded number that does not grow with the
    # number of transactions. Verified locally: 5 transfers (10 txns) and 20 transfers (40 txns) both
    # produce exactly 21 queries - constant, not the 15 originally guessed here without measuring.
    assert_operator queries.size, :<=, 25, "Expected bounded query count without N+1 regression, got #{queries.size} queries:\n#{queries.join("\n")}"
  end
