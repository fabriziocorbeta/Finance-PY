-- ============================================================
-- CD & Co CRM · Master Database Schema with RLS
-- Creates all necessary tables for the CRM with Row Level Security
-- Based on backend_security_architecture.md analysis
-- ============================================================

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
-- Enable pgcrypto for encryption (if needed for sensitive fields)
...
-- ==========================================
-- 3. LEADS TABLE (Enhanced with security fields)
-- ==========================================
...
-- Leads: Enhanced policy with team support for Pro/Enterprise
CREATE POLICY "leads_select_enhanced" ON public.leads
  FOR SELECT USING (
    -- User can always see their own leads
    user_id = (SELECT id FROM public.users WHERE auth_user_id = auth.uid())
    OR 
    -- For Pro/Enterprise plans, allow team access (placeholder for future team implementation)
    (
      SELECT s.plan 
...
-- Leads: Update policy (owners and team admins for Pro/Enterprise)
CREATE POLICY "leads_update_team" ON public.leads
  FOR UPDATE USING (
    -- Owner can update
    user_id = (SELECT id FROM public.users WHERE auth_user_id = auth.uid())
    OR
    -- Team admin can update team members' leads (placeholder for future team implementation)
    (
      SELECT s.plan 
      FROM public.subscriptions s 
      WHERE s.user_id = (SELECT id FROM public.users WHERE auth_user_id = auth.uid()) 
...
DROP TRIGGER IF EXISTS update_interacciones_log_updated_at ON public.interacciones_log;
CREATE TRIGGER update_interacciones_log_updated_at 
  BEFORE UPDATE ON public.interacciones_log 
  FOR EACH ROW 
  EXECUTE FUNCTION public.update_updated_at_column();
...
COMMENT ON TABLE public.leads IS 'Customer/prospect information with funnel tracking and encrypted PII fields';
...
COMMENT ON COLUMN public.leads.wa_id_encrypted IS 'Encrypted WhatsApp ID for PII protection';
COMMENT ON COLUMN public.leads.ig_handle_encrypted IS 'Encrypted Instagram handle for PII protection';
