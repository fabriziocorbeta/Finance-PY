...
function exportCsvFiltered() {
  ...
  txs.forEach(t => rows.push(`${t.date},${t.type==='income'?'Ingreso':'Gasto'},"${t.desc.replace(/"/g, '""')}",${t.amount},${t.cur||'$'},"${t.cat||''}"`));
...
function renderTxs(){
  ...
  txs.forEach(t=>{ 
    ...
    if(tC===dCur) { if(t.type==='income'||t.type==='transfer-in') curInc+=t.amount; else curExp+=t.amount; } 
  });
  let prevInc=0, prevExp=0;
  prevTxs.forEach(t=>{ const tC = t.cur||t.currency||dCur; if(tC===dCur) { if(t.type==='income'||t.type==='transfer-in') prevInc+=t.amount; else prevExp+=t.amount; } });
  ...
  expTxs.forEach(t=>{ 
    const isAdj = (t.desc||'').toLowerCase().includes('ajuste') || (t.cat||'').toLowerCase().includes('ajuste') || (t.cat === 'Otros Ingresos' && t.amount > 500000);
    if(!isAdj) { expSums[t.cat] = (expSums[t.cat]||0) + t.amount; totalExpAna+=t.amount; }
  });
  incTxs.forEach(t=>{ 
    const isAdj = (t.desc||'').toLowerCase().includes('ajuste') || (t.cat||'').toLowerCase().includes('ajuste') || (t.cat === 'Otros Ingresos' && t.amount > 500000);
    if(!isAdj) { incSums[t.cat] = (incSums[t.cat]||0) + t.amount; totalIncAna+=t.amount; }
  });
  ...
  if(g('tx-donut')){
    ...
    const top6 = sortedExp.slice(0,6);
    const others = sortedExp.slice(6).reduce((a,c)=>a+c[1], 0);
    
    let chartLabels = top6.map(c=>c[0]);
    let chartData = top6.map(c=>c[1]);
    let listHtml = '';
    ...
    if(others > 0) {
       ...
       const pct = ((others/totalExpAna)*100).toFixed(1);
       listHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0">
         <div style="display:flex;align-items:center;gap:8px">
           <div style="width:10px;height:10px;border-radius:50%;background:#5a5c69"></div>
           <span style="font-size:0.8rem;color:#e2e8f0;font-weight:500">Otros</span>
         </div>
         <div style="display:flex;align-items:center;gap:12px">
           <span style="font-family:var(--fm);font-size:0.8rem;color:#cbd5e1">${fmt(others, dCur)}</span>
           <span style="font-size:0.7rem;color:#94a3b8;width:35px;text-align:right">${pct}%</span>
         </div>
       </div>`;
    }
    ...
    if(g('tx-donut-list')){
      if(!top6.length) g('tx-donut-list').innerHTML = '<div style="color:var(--m3);text-align:center;font-size:0.8rem">Sin gastos registrados</div>';
      else g('tx-donut-list').innerHTML = listHtml;
    }
    ...
    if(window.Chart){
      ...
      txChartInstance = new Chart(g('tx-donut'), {
        type: 'doughnut',
        data: { labels: chartLabels, datasets: [{ data: chartData, backgroundColor: [...chartColors.slice(0,6), '#5a5c69'], borderWidth: 0}] },
  ...
  if(g('tx-bars-exp') && g('tx-bars-inc')){
    ...
    const renderBars = (container, moreContainer, sortedData, totalAna, isExp) => {
      ...
      const remaining = sortedData.length > 6 ? sortedData.length - 6 : 0;
      ...
      g(container).innerHTML = top6.map((c, i) => {
      ...
      if(remaining > 0) g(moreContainer).textContent = '+ ' + remaining + ' categorías más';
      else g(moreContainer).textContent = '';
    };
  ...
  tb.innerHTML=txs.map(tx=>{
    let accName = '—';
    if(tx.accountId) {
      const a=(S.accounts||[]).find(x=>x.id===tx.accountId);
      if(a) accName=a.name;
      else {
        const c=(S.cards||[]).find(x=>x.id===tx.accountId);
        if(c) accName='💳 '+c.name;
      }
    }
    ...
    <td class="mono" style="color:${amtColor}">${amtSign}${fmt(tx.amount, tCur)}</td>
    <td>${typePill}</td>
...
function openTxModal(type,id){
  ...
  if(id){const tx=S.txs.find(t=>t.id===id);if(tx){txType=tx.type;setTT(tx.type);g('tx-desc').value=tx.desc;g('tx-amt').value=tx.amount;g('tx-cur').value=tx.cur||'$';g('tx-cat').value=tx.cat;g('tx-date').value=tx.date;if(txAcc&&tx.accountId)txAcc.value=tx.accountId;}}
