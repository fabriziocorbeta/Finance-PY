...
function calculateGoalBalance(accId) {
  ...
  (S.txs || []).forEach(t => {
    if (t.accountId === accId) {
      if (t.type === 'income' || t.type === 'transfer-in') bal += parseFloat(t.amount);
      else if (t.type === 'expense' || t.type === 'transfer-out') bal -= parseFloat(t.amount);
    }
  });
  return bal;
}
...
function saveGoal() {
  ...
  if (editIds.goal) {
    ...
    if (idx !== -1) {
      goalData.completed = S.goals[idx].completed;
      S.goals[idx] = goalData;
    }
  } else {
