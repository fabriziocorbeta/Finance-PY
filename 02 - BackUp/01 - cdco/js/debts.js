...
function getCardUsed(cardId) {
  ...
  if(!c) return 0;
  let used = parseFloat(c.used || 0);
  (S.txs || []).forEach(tx => {
    if (tx.accountId === cardId) {
      const isInc = (tx.type === 'income' || tx.type === 'transfer-in' || (tx.desc || '').toLowerCase().includes('pago'));
      if (isInc) used -= parseFloat(tx.amount) || 0;
      else used += parseFloat(tx.amount) || 0;
    }
  });
...
function renderCardDetails(id) {
  ...
  const txs = (S.txs||[]).filter(t=>t.accountId === c.id).sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,5);
  const subs = (S.subscriptions||[]).filter(s=>s.accountId === c.id && s.active);
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:30px">
       <div>
          ...
          ${txs.length===0?'<div style="font-size:.7rem;color:var(--mu)">Sin gastos recientes</div>':txs.map(tx=>{
            const isInc = (tx.type === 'income' || tx.type === 'transfer-in' || (tx.desc || '').toLowerCase().includes('pago'));
            return `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px dashed var(--bg4)"><div><div style="font-size:.75rem;color:var(--cr)">${tx.desc}</div><div style="font-size:.65rem;color:var(--m3)">${fmtDate(tx.date)}</div></div><div style="font-size:.8rem;color:${isInc?'var(--pos)':'#d47a7a'}">${isInc?'+':'-'}${fmt(tx.amount,tx.cur)}</div></div>`}).join('')}
       </div>
       <div>
...
function saveCardPayment() {
  ...
  S.txs.push({
    id: 'tout-' + baseId,
    type: 'transfer-out',
    desc: `Pago de tarjeta: ${c.name}`,
    amount: amt, cur: c.cur||'$', cat: 'Pago de Tarjeta', date,
    accountId: fromId,
    transferPairId: baseId
  });
  S.txs.push({
    id: 'tin-' + baseId,
    type: 'transfer-in',
    desc: `Pago recibido desde: ${fromAcc?fromAcc.name:'Cuenta'}`,
    amount: amt, cur: c.cur||'$', cat: 'Pago de Tarjeta', date,
    accountId: payingCardId,
    transferPairId: baseId
  });
  
  lsave(); renderAll(); cm('pay-card-modal');
...
function saveDebt(){
  ...
  const debt={creditor,description,totalAmount,paidAmount,installments,paidInstallments,dueDate,cur,status};
  if(editDebtId){
    const i=S.debts.findIndex(d=>d.id===editDebtId);
    if(i>=0) S.debts[i]={...S.debts[i],...debt};
    toast('◆ Deuda actualizada');
  } else {
    S.debts.push({...debt,id:uid()});
    toast('◆ Deuda registrada');
  }
  lsave();renderAll();cm('debt-modal');
}
...
function markDebtPaid(id){
  ...
  lsave();renderAll();toast('◆ Deuda marcada como pagada');
}
