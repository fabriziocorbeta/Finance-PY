...
function patMonthlyFlow(months) {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    (S.txs || []).forEach(tx => {
      ...
      const isAdj = tx.desc.toLowerCase().includes('ajuste') || tx.cat.toLowerCase().includes('ajuste') || (tx.cat === 'Otros Ingresos' && tx.amount > 500000);
      if (isAdj) return;
      const amt = patToDom(tx.amount || 0, tx.cur || '$', dCur);
