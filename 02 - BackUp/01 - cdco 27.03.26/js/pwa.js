...
// Session check
async function chkSess(){if(!SB_ON)return;const{data:{session}}=await sb.auth.getSession();if(session){S.user=session.user;const name=session.user.user_metadata?.full_name||session.user.email.split('@')[0];lload();defaults();enterApp(name,'pro');}}
chkSess();
