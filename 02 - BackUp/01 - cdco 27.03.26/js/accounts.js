...
function getAccountBalance(accountId) {
  ...
  if (!acc) return 0;
  const aCur = acc.cur || acc.currency || '$';
  const fxRate = (typeof FX !== 'undefined' && FX.buy && FX.buy > 1000) ? FX.buy : 7300;
  
  let bal = parseFloat(acc.initialBalance) || 0;
  (S.txs || []).forEach(tx => {
    if (tx.accountId !== accountId) return;
    let amt = parseFloat(tx.amount) || 0;
    const tCur = tx.cur || tx.currency || aCur; 
    
    // Si la moneda del movimiento es diferente a la de la cuenta, convertimos
    if (tCur !== aCur) {
      if (aCur === '₲') amt = amt * fxRate; // USD to PYG
      else amt = amt / fxRate; // PYG to USD
    }
    
    if (tx.type === 'income' || tx.type === 'transfer-in')  bal += amt;
    if (tx.type === 'expense' || tx.type === 'transfer-out') bal -= amt;
  });
  return bal;
}
...
function saveAccount() {
  ...
  const acct = { name, type, bank, cur, initialBalance: init, notes };
  if (editAccountId) {
    const i = (S.accounts || []).findIndex(a => a.id === editAccountId);
    if (i >= 0) S.accounts[i] = { ...S.accounts[i], ...acct };
    // Handle direct balance adjustment
    ...
    if (newBalEl && newBalEl.value !== '') {
      ...
      if (Math.abs(diff) > 0.001) {
        S.txs.push({
          ...
          cur: cur,
          cat: diff > 0 ? 'Otros Ingresos' : 'Otros Gastos',
          date: today(),
    ...
    S.accounts.push({ ...acct, id: uid() });
    toast('◆ Cuenta registrada');
  }
  lsave(); renderAll(); cm('account-modal');
...
function saveTransfer() {
  ...
  S.txs.push({
    id: 'tout-' + baseId,
    type: 'transfer-out',
    desc: `⇄ Transferencia a ${toAcc ? toAcc.name : 'cuenta'}: ${note}`,
    amount: amt, cur, cat: 'Transferencia', date,
    accountId: fromId,
  ...
  S.txs.push({
    id: 'tin-' + baseId,
    type: 'transfer-in',
    desc: `⇄ Transferencia de ${fromAcc ? fromAcc.name : 'cuenta'}: ${note}`,
    amount: amt, cur, cat: 'Transferencia', date,
    accountId: toId,
    transferPairId: baseId
  });
  lsave(); renderAll(); cm('transfer-modal');
