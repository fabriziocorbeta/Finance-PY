...
async function getBackupStatus() {
  try {
    ...
    console.log('✅ Estado de backup:', data);
    return data;
  } catch (err) {
...
// 🎬 INICIALIZAR BACKUP UI
async function initBackupUI() {
  console.log('🔄 [Backup UI] Inicializando...');

  // Actualizar estado cuando se carga
  await updateBackupStatusDisplay();

  // Actualizar cada 3 minutos
  setInterval(updateBackupStatusDisplay, 3 * 60 * 1000);

  // Agregar listener al botón
  const btn = document.getElementById('btn-backup-now');
  if (btn) {
    btn.addEventListener('click', triggerBackupNow);
    console.log('✅ [Backup UI] Botón configurado');
  } else {
    console.warn('⚠️  [Backup UI] Botón no encontrado');
  }

  console.log('✅ [Backup UI] Inicializado');
}
