...
class Api::V1::FleetVehiclesController < Api::V1::BaseController
  ...
  def index
    ...
    @pagy, @fleet_vehicles = pagy(
      fleet_vehicles_scope.includes(fuel_logs: :fuel_log_lines).order(created_at: :desc, id: :asc),
  ...
  def update
    if @fleet_vehicle.update(extracted_params)
  ...
  private
    ...
    def set_fleet_vehicle
      @fleet_vehicle = fleet_vehicles_scope.find(params[:id])
    ...
    def extracted_params
      raw = params.key?(:fleet_vehicle) ? params.require(:fleet_vehicle) : params
      raw.permit(:plate, :brand, :model, :year, :status, :notes).to_h.symbolize_keys
    end
