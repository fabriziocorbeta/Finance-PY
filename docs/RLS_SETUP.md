...
Antes de bloquear consultas no coincidentes, podés validar la coincidencia en logs para identificar cualquier consulta de background job o rake task que no esté enviando el context `app.current_family_id`.
...
Desplegar la versión del código que incluye el concern `RowLevelSecurity` en `ApplicationController`. Cada request establecerá:
```sql
SET app.current_family_id = '<family_id_uuid>';
```
al autenticar la sesión del usuario en cada petición HTTP.
